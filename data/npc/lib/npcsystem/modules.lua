...
if Modules == nil then
	...
	function StdModule.promotePlayer(cid, message, keywords, parameters, node)
		...
		if player:isPremium() or not parameters.premium then
			...
			elseif player:getLevel() < parameters.level then
				npcHandler:say("Sinto muito, mas só posso promovê-lo quando você atingir o nível " .. parameters.level .. ".", cid)
			...
			npcHandler:say("Você precisa de uma conta premium para ser promovido.", cid)
		end
		npcHandler:resetNpc(cid)
		return true
	end
	...
	function StdModule.travel(cid, message, keywords, parameters, node)
		...
		if player:isPremium() or not parameters.premium then
			if player:isPzLocked() then
				npcHandler:say("Primeiro se livrar dessas manchas de sangue! Você não vai estragar o meu veículo!", cid)
			elseif parameters.level and player:getLevel() < parameters.level then
				npcHandler:say("Você deve atingir o nível " .. parameters.level .. " antes que eu possa deixar você ir lá.", cid)
			elseif not player:removeTotalMoney(parameters.cost) then
				npcHandler:say("Você não tem dinheiro suficiente.", cid)
			else
				npcHandler:say(parameters.msg or "Coloque as velas!", cid)
			...
			npcHandler:say("Sinto muito, mas você precisa de uma conta premium para viajar a bordo de nossos navios.", cid)
		end
		npcHandler:resetNpc(cid)
		return true
	end
	...
	function TravelModule.travel(cid, message, keywords, parameters, node)
		...
		module.npcHandler:say("Você quer viajar para " .. keywords[1] .. " por " .. cost .. " gold coins?", cid)
		return true
	end
