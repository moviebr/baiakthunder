...
function onStepIn(creature, item, position, fromPosition)
	...
	if item.actionid >= 1000 then
		if creature:getLevel() < item.actionid - 1000 then
			...
			creature:sendTextMessage(MESSAGE_INFO_DESCR, "O piso parece estar protegido contra invasores indesejados.")
		end
		return true
	end
	...
	if Tile(position):hasFlag(TILESTATE_PROTECTIONZONE) then
		...
		if depotItem then
			...
			creature:sendTextMessage(MESSAGE_STATUS_DEFAULT, "Seu depot contém " .. depotItems .. " ite" .. (depotItems > 1 and "ns." or "m."))
			return true
		end
	end
