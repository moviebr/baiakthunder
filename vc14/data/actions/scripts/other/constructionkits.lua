...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if fromPosition.x == CONTAINER_POSITION then
		player:sendTextMessage(MESSAGE_STATUS_SMALL, "Coloque o kit de construção no chão primeiro.")
	elseif not Tile(fromPosition):getHouse() then
		player:sendTextMessage(MESSAGE_STATUS_SMALL, "Você pode construir isso apenas dentro de uma casa.")
	else
		item:transform(kit)
