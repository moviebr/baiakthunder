...
bool Game::internalStartTrade(Player* player, Player* tradePartner, Item* tradeItem)
{
	...
	if (tradePartner->tradeState == TRADE_NONE) {
		std::ostringstream ss;
		ss << player->getName() << " quer trocar com voc�.";
...
std::string Game::getTradeErrorDescription(ReturnValue ret, Item* item)
{
	if (item) {
		if (ret == RETURNVALUE_NOTENOUGHCAPACITY) {
			std::ostringstream ss;
			ss << "Voc� n�o tem capacidade suficiente para transportar";
		...
		if (realHealthChange > 0 && !target->isInGhostMode()) {
			...
			for (Creature* spectator : spectators) {
				...
				if (tmpPlayer == attackerPlayer && attackerPlayer != targetPlayer) {
					ss.str({});
					ss << "Voc� cura " << target->getNameDescription() << " para " << damageString;
					...
					if (!attacker) {
						...
						ss << "Voc� foi curado por " << attacker->getNameDescription();
					}
				...
				for (Creature* spectator : spectators) {
					...
					if (tmpPlayer == attackerPlayer && attackerPlayer != targetPlayer) {
						...
						if (!attacker) {
							...
							ss << " devido ao seu pr�prio ataque.";
						} else {
...
bool Game::combatChangeMana(Creature* attacker, Creature* target, CombatDamage& damage)
{
	...
	if (manaChange > 0) {
		...
		for (Creature* spectator : spectators) {
			...
			if (tmpPlayer == attackerPlayer && attackerPlayer != targetPlayer) {
				ss.str({});
				ss << ucfirst(target->getNameDescription()) << " perde " << damageString << " mana due to your attack.";
				message.type = MESSAGE_STATUS_DEFAULT;
				message.text = ss.str();
			} else if (tmpPlayer == targetPlayer) {
				ss.str({});
				...
				if (spectatorMessage.empty()) {
					ss.str({});
					ss << ucfirst(target->getNameDescription()) << " perde " << damageString << " mana";
					if (attacker) {
						ss << " devido a ";
						if (attacker == target) {
							ss << (targetPlayer->getSex() == PLAYERSEX_FEMALE ? "seu pr�prio ataque" : "seu pr�prio ataque");
